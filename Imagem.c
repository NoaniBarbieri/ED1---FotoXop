...
Imagem *alocaImagem(int largura, int altura){
    int i;
    Imagem *img = mallocSafe(sizeof(Imagem));//utilizando img pois foi informado nas funções abaixo
...
Imagem* copiaImagem(Imagem *origem){
    int h, w;
    if(origem == NULL){
        printf("nao foi encontrado uma origem.\n");
        exit(1);
    }
    else{
        //definindo um ponteiro para o tipo imagem, que irá auxiliar na copia da imagem dedsejada
        Imagem *copiaIMG;
        copiaIMG = alocaImagem(origem->largura, origem->altura);

        copiaIMG->altura = origem->altura;
        copiaIMG->largura = origem->largura;

        for (h = 0; h < copiaIMG->altura; h++){
            for (w = 0; w < copiaIMG->largura; w++){
                copiaIMG->pixel[h][w].cor[RED] = origem->pixel[h][w].cor[RED];
                copiaIMG->pixel[h][w].cor[GREEN] = origem->pixel[h][w].cor[GREEN];
                copiaIMG->pixel[h][w].cor[BLUE] = origem->pixel[h][w].cor[BLUE];
            }
        }
        return copiaIMG;
    }
...
void salvaImagem(Imagem *img, const char *nomeArquivo){
    ...
    while(i< img->altura){
        while(j < img->largura){
            fprintf(arquivo, "%d ", img->pixel[i][j].cor[RED]);
            fprintf(arquivo, "%d ", img->pixel[i][j].cor[GREEN]);
